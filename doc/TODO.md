...
- [ ] MCP annotate/get_annotations resolve against the backend active tab, not the passed `tabId`: `AddAnnotations`/`GetRowAnnotations` use `GetActiveTab()` (app.go, workspace/local.go) while the MCP handler passes a specific `tabId`. Annotating a non-active (or not-yet-loaded) tab over MCP writes against the wrong tab's query rows. Pre-existing; fine for the UI (you annotate the visible tab) but a sharp edge for MCP automation. Make the MCP path activate/await the target tab or resolve by tabId
